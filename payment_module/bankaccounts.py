...
class BankAccounts:
    ...
    def add_log(txt:str):
        ...
        with open(BankAccounts.log_file,'a') as f:
            f.write(txt+f" _ at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"+os.linesep)
    ...
    def get_bank_accounts(user_id:str):
        ...
        return r
    @staticmethod
    def get_bank_account_balance(user_id:str,account_name:str,cvv:int , password_account:str):
        ...
        if cvv != r[0][0] or not bcrypt.checkpw(password_account.encode('utf-8'),r[0][1].encode('utf-8')):
            raise personalized_exceptions.InvalidAccountSecurityInformation()
        
        return r[0][2]+" : "+str(r[0][3])
        
    @staticmethod
    def deposit_to_bank_account(user_id:str,account_name:str,amount:int)->bool:
        ...
        id = r[0][0]
        balance = r[0][1]
        query = f"""UPDATE `cinemaswift`.`bankaccounts` SET `balance` = '{balance+amount}' WHERE (`id` = '{id}');"""
        BankAccounts.database_manager.execute_query(query)
        
        BankAccounts.add_log(f"Deposit _ {user_id = } _ {account_name = } _ {amount = }")
        
        return True
    
    @staticmethod
    def harvest_from_bank_account(user_id:str,cvv:int,password_account:str,amount:float,account_name:str)->bool:
        ...
        account = models.bank_account_model(r[0][0],r[0][1],r[0][2],r[0][3],r[0][4],r[0][5])
        
        if cvv != account.cvv2 or not bcrypt.checkpw(password_account.encode('utf-8'),r[0][5].encode('utf-8')):
            raise personalized_exceptions.InvalidAccountSecurityInformation()
        if account.balance < amount:
            raise personalized_exceptions.NotEnoughBalance()
        
        query = f"""UPDATE `cinemaswift`.`bankaccounts` SET `balance` = '{account.balance-amount}' 
                WHERE 
                (`id` = '{account.id}');"""
        BankAccounts.database_manager.execute_query(query)
        
        BankAccounts.add_log(f"harvest _ {user_id = } _ {account_name = } _ {amount = }")
        
        return True
        
    @staticmethod
    def money_transfer(user_id_origin:str,account_name_origin:str,cvv:int,password:str,
                       user_id_destination:str,account_name_destination:str,amount:int):
        
        BankAccounts.harvest_from_bank_account(user_id_origin,cvv,password,amount,account_name_origin)
        
        BankAccounts.deposit_to_bank_account(user_id_destination,account_name_destination,amount)
